...
export function ProfessionalClassicTemplate({ data }: TemplateProps) {
	...
	return (
		<div style={{ fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '10pt', lineHeight: 1.4, color: '#222', display: 'flex', minHeight: '100%' }}>
			{/* Left sidebar */}
			<div style={{ width: '30%', backgroundColor: NAVY_LIGHT, padding: '0.4in 0.3in', boxSizing: 'border-box' as const }}>
				<h1 style={{ fontSize: '16pt', fontWeight: 700, color: NAVY, margin: '0 0 2pt 0' }}>
					{contactInfo.name}
				</h1>
				{targetTitle && <p style={{ fontSize: '9.5pt', color: '#555', margin: '0 0 12pt 0', fontWeight: 600 }}>{targetTitle}</p>}
				...
				{sidebarSections.map((section) => {
					switch (section) {
						case 'skills':
							return data.skills.length > 0 ? <SidebarSkills key={section} items={data.skills} /> : null;
						case 'certifications':
							return data.certifications.length > 0 ? <SidebarCertifications key={section} items={data.certifications} /> : null;
						case 'awards':
							return data.awards.length > 0 ? <SidebarAwards key={section} items={data.awards} /> : null;
						default:
							return null;
					}
				})}
			...
			<div style={{ width: '70%', padding: '0.4in 0.35in', boxSizing: 'border-box' as const }}>
				{mainSections.map((section) => {
					switch (section) {
						...
						case 'skills':
							return data.skills.length > 0 ? <MainSkills key={section} items={data.skills} /> : null;
						case 'certifications':
							return data.certifications.length > 0 ? <MainCerts key={section} items={data.certifications} /> : null;
						case 'awards':
							return data.awards.length > 0 ? <MainAwards key={section} items={data.awards} /> : null;
						case 'projects':
...
function Bullet({ children }: { children: React.ReactNode }) {
	return <p style={{ margin: '1pt 0 1pt 10pt', textIndent: '-8pt', fontSize: '9.5pt' }}>• {children}</p>;
}
		...
		<>
			...
			{items.map((item, i) => (
				<div key={i} style={{ fontSize: '9pt', marginBottom: '3pt' }}>
					...
					{item.issuer && <div style={{ color: '#555' }}>{item.issuer}{item.issueDate ? ` · ${formatDate(item.issueDate)}` : ''}</div>}
				</div>
			))}
		</>
	);
		...
		<>
			...
			{items.map((item, i) => (
				<div key={i} style={{ fontSize: '9pt', marginBottom: '3pt' }}>
					...
					{item.issuer && <div style={{ color: '#555' }}>{item.issuer}{item.date ? ` · ${formatDate(item.date)}` : ''}</div>}
				</div>
			))}
		</>
	);
			...
			{items.map((item, i) => (
				<div key={i} style={{ marginBottom: '10pt' }}>
					<div style={{ display: 'flex', justifyContent: 'space-between' as const }}>
						...
						<span style={{ fontSize: '9.5pt', color: '#555' }}>{dateRange(item.startDate, item.endDate)}</span>
					</div>
					<div style={{ fontSize: '9.5pt', color: NAVY, fontWeight: 600, marginBottom: '2pt' }}>{item.companyName}</div>
					{item.technologies && item.technologies.length > 0 && (
						<div style={{ fontSize: '8.5pt', color: '#666', marginBottom: '2pt' }}>{item.technologies.join(' · ')}</div>
					)}
			...
			{items.map((item, i) => (
				<div key={i} style={{ marginBottom: '4pt' }}>
					...
					{(item.publisher || item.date) && (
						<span style={{ color: '#555' }}> ({[item.publisher, item.date && formatDate(item.date)].filter(Boolean).join(', ')})</span>
					)}
				</div>
			))}
		</>
