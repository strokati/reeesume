...
export function ModernMinimalTemplate({ data }: TemplateProps) {
	...
	return (
		<div style={{ fontFamily: '"Segoe UI", Roboto, Helvetica, Arial, sans-serif', fontSize: '10.5pt', lineHeight: 1.5, color: '#1a1a1a', padding: '0.5in', maxWidth: '8.5in', boxSizing: 'border-box' as const }}>
			{/* Name */}
			{contactInfo.name && (
				<h1 style={{ fontSize: '26pt', fontWeight: 300, margin: '0 0 4pt 0', letterSpacing: '1pt', paddingBottom: '6pt', borderBottom: `2px solid ${accent}` }}>
					{contactInfo.name}
				</h1>
			)}

			{/* Target title */}
			{targetTitle && <p style={{ fontSize: '11pt', fontWeight: 500, color: accent, margin: '4pt 0 6pt 0' }}>{targetTitle}</p>}

			{/* Contact */}
			{contactParts.length > 0 && (
				<p style={{ fontSize: '9pt', color: '#666', margin: '0 0 16pt 0' }}>{contactParts.join('  ·  ')}</p>
			)}
			...
			{sectionOrder.map((section) => {
				switch (section) {
					case 'summary':
						return summary ? <SummarySection key={section} summary={summary} accent={accent} /> : null;
					case 'workExperience':
						return data.workExperience.length > 0 ? <WorkSection key={section} items={data.workExperience} accent={accent} /> : null;
					case 'education':
						return data.education.length > 0 ? <EducationSection key={section} items={data.education} accent={accent} /> : null;
					case 'skills':
						return data.skills.length > 0 ? <SkillsSection key={section} items={data.skills} accent={accent} /> : null;
					case 'certifications':
						return data.certifications.length > 0 ? <CertsSection key={section} items={data.certifications} accent={accent} /> : null;
					case 'awards':
						return data.awards.length > 0 ? <AwardsSection key={section} items={data.awards} accent={accent} /> : null;
					case 'projects':
						return data.projects.length > 0 ? <ProjectsSection key={section} items={data.projects} accent={accent} /> : null;
					case 'volunteering':
						return data.volunteering.length > 0 ? <VolunteeringSection key={section} items={data.volunteering} accent={accent} /> : null;
					case 'publications':
						return data.publications.length > 0 ? <PublicationsSection key={section} items={data.publications} accent={accent} /> : null;
					default:
						return null;
				}
			})}
			...
			{items.map((item, i) => (
				<div key={i} style={{ marginBottom: '10pt' }}>
					<div style={{ display: 'flex', justifyContent: 'space-between' as const, alignItems: 'baseline' as const }}>
						...
						<span style={{ fontSize: '9pt', color: '#888' }}>{dateRange(item.startDate, item.endDate)}</span>
					</div>
					<div style={{ fontSize: '9.5pt', color: accent, fontWeight: 500 }}>{item.companyName}</div>
					{item.technologies && item.technologies.length > 0 && (
						<div style={{ margin: '2pt 0', fontSize: '8.5pt', color: '#666' }}>{item.technologies.join(' · ')}</div>
					)}
		...
		<>
			...
			{items.map((item, i) => (
				<div key={i} style={{ marginBottom: '8pt' }}>
					...
					{item.description && <p style={{ margin: '2pt 0', color: '#444', fontSize: '9.5pt' }}>{item.description}</p>}
					{item.technologies && item.technologies.length > 0 && (
		...
		<>
			...
			{items.map((item, i) => (
				<div key={i} style={{ marginBottom: '6pt' }}>
					...
					<div style={{ fontSize: '9.5pt', color: accent, fontWeight: 500 }}>{item.organization}</div>
			...
			{items.map((item, i) => (
				<div key={i} style={{ marginBottom: '4pt' }}>
					...
					{(item.publisher || item.date) && (
						<span style={{ color: '#888' }}> ({[item.publisher, item.date && formatDate(item.date)].filter(Boolean).join(', ')})</span>
					)}
				</div>
			))}
		</>
