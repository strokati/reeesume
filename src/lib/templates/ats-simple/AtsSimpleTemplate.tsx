	...
	return (
		<div style={STYLES.page}>
			...
			{contactParts.length > 0 && (
				<p style={STYLES.contactLine}>{contactParts.join(' | ')}</p>
			)}
				...
				<div key={i} style={{ marginBottom: '8pt' }}>
					<div style={STYLES.entryHeader}>
						...
						{item.startDate || item.endDate ? (
							<span style={STYLES.entryMetaRight}>{formatDataRange(item.startDate, item.endDate)}</span>
						) : null}
					</div>
		...
		<>
			...
			{items.map((item, i) => (
				<div key={i} style={{ marginBottom: '4pt' }}>
					...
					{item.url && (
						<div style={{ fontSize: '9.5pt', margin: '1pt 0' }}>{item.url}</div>
					)}
				</div>
			))}
		</>
