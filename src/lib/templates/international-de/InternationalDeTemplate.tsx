...
export function InternationalDeTemplate({ data }: TemplateProps) {
	...
	const deSectionOrder = ['workExperience', 'education', 'skills', 'certifications', 'awards', 'projects', 'volunteering', 'publications'];

	return (
		<div style={{ fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '10pt', lineHeight: 1.45, color: '#222', padding: '0.5in', maxWidth: '8.5in', boxSizing: 'border-box' as const }}>
			{/* Header with photo */}
			<div style={{ display: 'flex', marginBottom: '12pt' }}>
				<div style={{ flex: 1 }}>
					{contactInfo.name && <h1 style={{ fontSize: '18pt', fontWeight: 700, margin: '0 0 4pt 0', color: '#1a1a1a' }}>{contactInfo.name}</h1>}
					{targetTitle && <p style={{ fontSize: '11pt', color: '#444', margin: '0 0 8pt 0', fontWeight: 500 }}>{targetTitle}</p>}
				</div>
				<div style={{
					width: '100px',
					height: '125px',
					...
					{contactInfo.photoUrl ? (
						...
						<img src={contactInfo.photoUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' as const }} />
					) : (
						'Photo'
					)}
				</div>
			...
			<table style={{ fontSize: '9.5pt', borderCollapse: 'collapse' as const, marginBottom: '4pt' }}>
				<tbody>
					...
					{contactInfo.dateOfBirth && <ContactRow label="Geburtsdatum" value={formatDeDate(contactInfo.dateOfBirth)} />}
					{contactInfo.nationality && <ContactRow label="Staatsangehörigkeit" value={contactInfo.nationality} />}
					{contactInfo.maritalStatus && <ContactRow label="Familienstand" value={contactInfo.maritalStatus} />}
			...
			{deSectionOrder.map((section) => {
				...
				switch (section) {
					case 'workExperience':
						return data.workExperience.length > 0 ? <WorkSection key={section} items={data.workExperience} /> : null;
					case 'education':
						return data.education.length > 0 ? <EducationSection key={section} items={data.education} /> : null;
					case 'skills':
						return data.skills.length > 0 ? <SkillsSection key={section} items={data.skills} /> : null;
					case 'certifications':
						return data.certifications.length > 0 ? <CertsSection key={section} items={data.certifications} /> : null;
					case 'awards':
						return data.awards.length > 0 ? <AwardsSection key={section} items={data.awards} /> : null;
					case 'projects':
						return data.projects.length > 0 ? <ProjectsSection key={section} items={data.projects} /> : null;
					case 'volunteering':
						return data.volunteering.length > 0 ? <VolunteeringSection key={section} items={data.volunteering} /> : null;
					case 'publications':
						return data.publications.length > 0 ? <PublicationsSection key={section} items={data.publications} /> : null;
					default:
						return null;
				}
			})}
...
function ContactRow({ label, value }: { label: string; value?: string }) {
	...
	return (
		<tr>
			<td style={{ padding: '1pt 8pt 1pt 0', fontWeight: 600, color: '#555', verticalAlign: 'top' as const, whiteSpace: 'nowrap' as const }}>{label}</td>
			...
			{items.map((item, i) => (
				<div key={i} style={{ marginBottom: '8pt' }}>
					<div style={{ display: 'flex', justifyContent: 'space-between' as const }}>
						...
						<span style={{ fontSize: '9.5pt', color: '#555' }}>{deDateRange(item.startDate, item.endDate)}</span>
					</div>
					<div style={{ fontSize: '9.5pt', color: '#444', fontWeight: 500, marginBottom: '2pt' }}>{item.companyName}</div>
					{item.technologies && item.technologies.length > 0 && (
						<div style={{ fontSize: '8.5pt', color: '#666', margin: '1pt 0' }}>{item.technologies.join(', ')}</div>
					)}
					{item.responsibilities.map((r, j) => <Bullet key={`r-${j}`}>{r}</Bullet>)}
					{item.achievements.map((a, j) => <Bullet key={`a-${j}`}>{a}</Bullet>)}
				</div>
			))}
		</>
	);
				...
				<div key={i} style={{ marginBottom: '6pt' }}>
					<div style={{ display: 'flex', justifyContent: 'space-between' as const }}>
						<span style={{ fontWeight: 700 }}>
							{item.degree && item.field ? `${item.degree} in ${item.field}` : item.degree || item.field}
						</span>
						<span style={{ fontSize: '9.5pt', color: '#555' }}>{deDateRange(item.startDate, item.endDate)}</span>
					</div>
		...
		<>
			...
			{items.map((item, i) => (
				<div key={i} style={{ marginBottom: '3pt' }}>
					...
					{item.issueDate && <span style={{ color: '#555' }}> ({formatDeDate(item.issueDate)})</span>}
				</div>
			))}
		</>
	);
		...
		<>
			...
			{items.map((item, i) => (
				<div key={i} style={{ marginBottom: '6pt' }}>
					...
					{item.description && <p style={{ margin: '2pt 0', fontSize: '9.5pt' }}>{item.description}</p>}
					{item.technologies && item.technologies.length > 0 && (
		...
		<>
			...
			{items.map((item, i) => (
				<div key={i} style={{ marginBottom: '6pt' }}>
					...
					{item.responsibilities?.map((r, j) => <Bullet key={j}>{r}</Bullet>)}
				</div>
			))}
		</>
	);
			...
			{items.map((item, i) => (
				<div key={i} style={{ marginBottom: '4pt' }}>
					...
					{(item.publisher || item.date) && (
						<span style={{ color: '#555' }}> ({[item.publisher, item.date && formatDeDate(item.date)].filter(Boolean).join(', ')})</span>
					)}
				</div>
			))}
		</>
