		...
		switch (section) {
			case 'summary':
				if (data.summary) {
					...
					children.push(new Paragraph({ children: [new TextRun({ text: data.summary, size: 21 })], spacing: { after: 150 } }));
				}
				break;
			...
			case 'workExperience':
				if (data.workExperience.length > 0) {
					...
					for (const item of data.workExperience) {
						const titleChildren: TextRun[] = [new TextRun({ text: item.title, bold: true, size: 21 })];
						...
						children.push(new Paragraph({ children: [new TextRun({ text: item.companyName, size: 21, color: '333333' })] }));
						if (item.technologies?.length) {
							children.push(new Paragraph({ children: [new TextRun({ text: `Technologies: ${item.technologies.join(', ')}`, size: 19, color: '444444' })], spacing: { after: 40 } }));
						}
			...
			case 'education':
				if (data.education.length > 0) {
					...
					for (const item of data.education) {
						const degreeText = item.degree && item.field ? `${item.degree} in ${item.field}` : item.degree || item.field || '';
						const eduChildren: TextRun[] = [new TextRun({ text: degreeText, bold: true, size: 21 })];
						const dateRange = formatDataRange(item.startDate, item.endDate);
						...
						const institution = item.location ? `${item.institution}, ${item.location}` : item.institution;
						children.push(new Paragraph({ children: [new TextRun({ text: institution, size: 21, color: '333333' })] }));
						const extra: string[] = [];
						if (item.gpa) extra.push(`GPA: ${item.gpa}`);
						if (item.honors) extra.push(item.honors);
						if (extra.length) {
							children.push(new Paragraph({ children: [new TextRun({ text: extra.join(' | '), size: 19 })] }));
						}
					}
				}
				break;
			...
			case 'projects':
				if (data.projects.length > 0) {
					...
					for (const item of data.projects) {
						const projChildren: TextRun[] = [new TextRun({ text: item.name, bold: true, size: 21 })];
						if (item.role) projChildren.push(new TextRun({ text: ` — ${item.role}`, size: 21, color: '333333' }));
						children.push(new Paragraph({ children: projChildren, spacing: { before: 100 } }));
						if (item.description) {
							children.push(new Paragraph({ children: [new TextRun({ text: item.description, size: 21 })], spacing: { after: 40 } }));
						}
						if (item.technologies?.length) {
							children.push(new Paragraph({ children: [new TextRun({ text: `Technologies: ${item.technologies.join(', ')}`, size: 19, color: '444444' })] }));
						}
					}
				}
				break;
			...
			case 'volunteering':
				if (data.volunteering.length > 0) {
					...
					for (const item of data.volunteering) {
						...
						children.push(new Paragraph({ children: [new TextRun({ text: item.organization, size: 21, color: '333333' })] }));
