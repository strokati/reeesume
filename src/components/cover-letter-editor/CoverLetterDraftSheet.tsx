			...
			<SheetContent className="w-96">
				...
				<div className="mt-4 space-y-3">
					...
					{sorted.map((draft) => {
						...
						return (
							<div key={draft.id} className={`rounded-xl border p-3 space-y-2 ${isActive ? 'border-primary bg-primary/5' : ''}`}>
								<div className="flex items-start justify-between gap-2">
									{renamingId === draft.id ? (
										<Input value={renameValue} onChange={(e) => setRenameValue(e.target.value)} onBlur={() => handleRename(draft.id)} onKeyDown={(e) => { if (e.key === 'Enter') handleRename(draft.id); }} autoFocus className="h-7 text-sm" />
									) : (
										<button type="button" className="text-sm font-medium text-left hover:underline" onClick={() => { setRenamingId(draft.id); setRenameValue(draft.name); }}>
											{draft.name}
										</button>
									)}
									<Badge variant="outline" className={`text-[0.6rem] ${statusColors[draft.status] ?? ''}`}>{draft.status}</Badge>
								</div>
								<div className="text-xs text-muted-foreground">
									{new Date(draft.createdAt).toLocaleDateString()} · {draft.tone}
								</div>
								<div className="flex gap-1">
									{!isActive && <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => handleSetActive(draft.id)}><Star className="h-3 w-3 mr-1" />Set Active</Button>}
									<Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => handleDuplicate(draft.id)}><Copy className="h-3 w-3 mr-1" />Duplicate</Button>
									{drafts.length > 1 && <Button variant="ghost" size="sm" className="h-7 text-xs text-muted-foreground hover:text-red-600" onClick={() => handleDelete(draft.id)}><Trash2 className="h-3 w-3" /></Button>}
								</div>
							</div>
						);
					})}
