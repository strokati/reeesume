...
export function CertificationCard({ certification, resumeId, dragHandleProps }: {
	certification: Certification; resumeId: string; dragHandleProps?: Record<string, unknown>;
}) {
	...
	function handleDelete() {
		startTransition(async () => {
			try { await deleteCertification(certification.id); toast.success('Certification deleted'); }
	...
	return (
		<>
			<Card>
				<CardHeader className="flex flex-row items-center justify-between space-y-0 p-3">
					<div className="flex items-center gap-2 min-w-0">
						...
						{certification.issuer && <span className="text-sm text-muted-foreground truncate">{certification.issuer}</span>}
					</div>
					<div className="flex items-center gap-1 shrink-0">
						{certification.url && (
							<a href={certification.url} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground"><ExternalLink className="h-3 w-3" /></a>
						)}
			...
			<CertificationDialog open={editOpen} onOpenChange={setEditOpen} resumeId={resumeId} certification={certification} />
		</>
	);
}
