...
export function PublicationCard({ publication, resumeId, dragHandleProps }: {
	publication: Publication; resumeId: string; dragHandleProps?: Record<string, unknown>;
}) {
	...
	function handleDelete() {
		startTransition(async () => {
			try { await deletePublication(publication.id); toast.success('Deleted'); }
	...
	return (
		<>
			<Card>
				<CardHeader className="flex flex-row items-center justify-between space-y-0 p-3">
					<div className="flex items-center gap-2 min-w-0">
						...
						{publication.authors && <span className="text-sm text-muted-foreground truncate">{publication.authors}</span>}
					</div>
					<div className="flex items-center gap-1 shrink-0">
						{(publication.url || publication.doi) && (
							<a href={publication.url || `https://doi.org/${publication.doi}`} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground">
			...
			<PublicationDialog open={editOpen} onOpenChange={setEditOpen} resumeId={resumeId} publication={publication} />
		</>
	);
}
