...
export function AwardCard({ award, resumeId, dragHandleProps }: { award: Award; resumeId: string; dragHandleProps?: Record<string, unknown> }) {
	...
	function handleDelete() {
		startTransition(async () => {
			try { await deleteAward(award.id); toast.success('Award deleted'); }
			...
			<Card>
				<CardHeader className="flex flex-row items-center justify-between space-y-0 p-3">
					<div className="flex items-center gap-2 min-w-0">
						...
						{award.issuer && <span className="text-sm text-muted-foreground truncate">{award.issuer}</span>}
				...
				{award.description && (
					<CardContent className="px-3 pb-3 pt-0 text-sm text-muted-foreground line-clamp-2">{award.description}</CardContent>
				)}
			</Card>
