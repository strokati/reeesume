...
function SortableProjectCard({ project, resumeId }: { project: Project; resumeId: string }) {
	...
	return (
		<div ref={setNodeRef} style={style} className={isDragging ? 'opacity-50' : undefined}>
			<ProjectCard project={project} resumeId={resumeId} dragHandleProps={{ ...attributes, ref: setActivatorNodeRef, ...listeners }} />
		</div>
	);
}
...
export function ProjectSection({ projects, resumeId }: { projects: Project[]; resumeId: string }) {
	...
	function handleDragEnd(event: DragEndEvent) {
		...
		startTransition(async () => {
			try { await reorderProjects(resumeId, reordered.map((p) => p.id)); }
			catch { toast.error('Failed to reorder'); }
		});
	}

	return (
		<SectionCard title="Projects" collapsible action={
				...
				<DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
					<SortableContext items={projects.map((p) => p.id)} strategy={verticalListSortingStrategy}>
						<div className="space-y-2">
							{projects.map((p) => <SortableProjectCard key={p.id} project={p} resumeId={resumeId} />)}
						</div>
					</SortableContext>
				</DndContext>
			)}
