...
function SortableVolunteeringCard({ role, resumeId }: { role: VolunteeringRole; resumeId: string }) {
	const { attributes, listeners, setNodeRef, setActivatorNodeRef, transform, transition, isDragging } = useSortable({ id: role.id });
	const style = { transform: CSS.Transform.toString(transform), transition };
	return (
		<div ref={setNodeRef} style={style} className={isDragging ? 'opacity-50' : undefined}>
			<VolunteeringCard volunteeringRole={role} resumeId={resumeId} dragHandleProps={{ ...attributes, ref: setActivatorNodeRef, ...listeners }} />
		</div>
	);
}

export function VolunteeringSection({ roles, resumeId }: { roles: VolunteeringRole[]; resumeId: string }) {
	...
	function handleDragEnd(event: DragEndEvent) {
		...
		startTransition(async () => {
			try { await reorderVolunteeringRoles(resumeId, reordered.map((r) => r.id)); }
			catch { toast.error('Failed to reorder'); }
		});
	}

	return (
		<SectionCard title="Volunteering & Leadership" collapsible action={
				...
				<DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
					<SortableContext items={roles.map((r) => r.id)} strategy={verticalListSortingStrategy}>
						<div className="space-y-2">
							{roles.map((r) => <SortableVolunteeringCard key={r.id} role={r} resumeId={resumeId} />)}
						</div>
					</SortableContext>
				</DndContext>
			)}
