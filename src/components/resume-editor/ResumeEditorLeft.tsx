...
function SourceBadge({ source }: { source: ContentSource }) {
	const styles: Record<ContentSource, string> = {
		master: 'border-blue-200 text-blue-700 bg-blue-50 dark:border-blue-800 dark:text-blue-300 dark:bg-blue-900/20',
		ai: 'border-orange-200 text-orange-700 bg-orange-50 dark:border-orange-800 dark:text-orange-300 dark:bg-orange-900/20',
		manual: 'border-green-200 text-green-700 bg-green-50 dark:border-green-800 dark:text-green-300 dark:bg-green-900/20',
	};
	const labels: Record<ContentSource, string> = { master: 'Master', ai: 'AI', manual: 'Edited' };

	return (
		<span className={`inline-flex items-center rounded-full border px-1.5 py-0.5 text-[0.55rem] font-medium ${styles[source]}`}>
			{labels[source]}
		</span>
	);
}
...
export function ResumeEditorLeft({ draft }: { draft: ResumeDraft }) {
	const content = (draft.content as unknown as ResumeDraftContent) ?? { workExperience: [], education: [], skills: [], certifications: [], awards: [], projects: [], volunteering: [], publications: [] };
	const [localContent, setLocalContent] = useState<ResumeDraftContent>(content);
	const saveTimeoutRef = useRef<NodeJS.Timeout | null>(null);

	const debouncedSave = useCallback((updated: ResumeDraftContent) => {
	...
	return (
		<div className="space-y-4 max-h-[calc(100vh-12rem)] overflow-y-auto pr-1">
			...
			{localContent.workExperience.length > 0 && (
				<Card>
					<CardContent className="p-4 space-y-4">
						<h3 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Work Experience</h3>
			...
			{localContent.volunteering.length > 0 && (
				<Card>
					<CardContent className="p-4 space-y-2">
						<h3 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Volunteering</h3>
						{localContent.volunteering.map((v, i) => (
							<div key={i} className="flex items-start justify-between text-sm">
								<span>{v.role ?? 'Volunteer'} at {v.organization}</span>
			...
			{localContent.publications.length > 0 && (
				<Card>
					<CardContent className="p-4 space-y-2">
						<h3 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Publications</h3>
...
function DraftWorkRoleCard({
	...
	function handleRephrase(index: number, rephrased: string, field: 'responsibilities' | 'achievements') {
		const updated = [...role[field]];
		...
		<div className="space-y-2 rounded-xl border p-3">
			...
			{role.responsibilities.length > 0 && (
				<div className="space-y-1">
					<p className="text-[0.65rem] uppercase tracking-wide text-muted-foreground font-medium">Responsibilities</p>
			...
			{role.achievements.length > 0 && (
				<div className="space-y-1">
					<p className="text-[0.65rem] uppercase tracking-wide text-muted-foreground font-medium">Achievements</p>
