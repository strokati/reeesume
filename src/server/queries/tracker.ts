...
export async function getTrackerData(userId: string): Promise<TrackerRow[]> {
	const applications = await db.application.findMany({
		...
		include: {
			vacancy: { select: { id: true, jobTitle: true, companyName: true, location: true, sourceUrl: true } },
			resumeDrafts: {
				where: { isActive: true },
	...
	return applications.map((app) => {
		...
		if (activeResume) {
			resumeStatus = activeResume.status === 'ready' || activeResume.status === 'exported' ? 'ready' : 'draft';
		}

		let coverLetterStatus: TrackerRow['coverLetterStatus'] = 'none';
		if (activeCover) {
			coverLetterStatus = activeCover.status === 'ready' || activeCover.status === 'exported' ? 'ready' : 'draft';
		}
