	...
	return {
		...
		workExperience: (full.workCompanies ?? []).flatMap((c) =>
			c.roles.map((r) => ({
				...
				responsibilities: ((r.responsibilities as string[] | null) ?? []).map((t) => ({ text: t, source: 'master' as const })),
				achievements: ((r.achievements as string[] | null) ?? []).map((t) => ({ text: t, source: 'master' as const })),
		...
		volunteering: (full.volunteeringRoles ?? []).map((v) => ({
			...
			responsibilities: ((v.responsibilities as string[] | null) ?? []).map((t) => ({ text: t, source: 'master' as const })),
			source: 'master' as const,
		})),
...
export async function updateResumeDraftContent(
	...
	await requireAuth();
	try {
		await db.resumeDraft.update({ where: { id }, data: { content: JSON.parse(JSON.stringify(content)) } });
	} catch {
		throw new Error('Failed to update draft content.');
	}
}

export async function updateResumeDraftTemplate(
	id: string,
	templateId: string,
): Promise<void> {
	await requireAuth();
